...
if (!empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])) {
    ...
    $query .= " AND d.tgl_domisili BETWEEN '$dari_tanggal' AND '$sampai_tanggal'";
}

// Urutkan berdasarkan tanggal pengajuan terbaru
$query .= " ORDER BY d.tgl_domisili DESC";
...
<head>
    ...
    <title>Desa Baluti - Domisili</title>
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <div class="card">
                                ...
                                <form method="GET" action="">
                                    <div class="row align-items-end">
                                        ...
                                        <div class="col-md-3">
                                            <label>&nbsp;</label>
                                            <div class="d-flex">
                                                <button type="submit" class="btn btn-primary me-2 w-100"><i class="fa fa-search"></i> Cari</button>
                                                <a href="<?= strtok($_SERVER["REQUEST_URI"], '?') ?>" class="btn btn-secondary w-100">
                                                    <i class="fa fa-refresh"></i> Reset
                                                </a>
                                            </div>
                                        </div>
                                        <?php if (!empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])): ?>
                                            <div class="col-md-3">
                                                <label>&nbsp;</label>
                                                <a href="domisili-print.php?dari_tanggal=<?= $_GET['dari_tanggal'] ?>&sampai_tanggal=<?= $_GET['sampai_tanggal'] ?>"
                            ...
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="mb-0">Data Domisili</h5>
                                <button class="btn btn-secondary" onclick="window.location.href='form-domisili.php'"><i class="mdi mdi-pen-plus me-1"></i>Buat Surat Domisili</button>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        if ($result->num_rows > 0) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                echo "<td>" . htmlspecialchars($row['alamat_asal']) . "</td>";
                                                echo "<td>" . htmlspecialchars($row['alamat_tujuan']) . "</td>";
                                                echo "<td>" . (isset($row['no_surat']) ? htmlspecialchars($row['no_surat']) : '-') . "</td>";
                                                echo "<td>" . (!empty($row['tgl_domisili']) ? date("d-m-Y", strtotime($row['tgl_domisili'])) : '') . "</td>";
                                                echo "<td>" . (isset($row['ket']) ? htmlspecialchars($row['ket']) : '-') . "</td>";
                                                echo "<td>
                                                    <a href='edit-domisili.php?id_domisili=" . $row['id_domisili'] . "' class='btn btn-primary mr-1'><i class='fa fa-pencil'></i></a>
                                                    <a href='hapus-domisili.php?id_domisili=" . $row['id_domisili'] . "' class='btn btn-danger mr-1 ' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'><i class='fa fa-trash'></i></a>
                                                    <a href='print-domisili.php?id_domisili=" . $row['id_domisili'] . "' class='btn btn-success mr-1' target='_blank' onclick='openPrint(this.href)'><i class='fa fa-print'></i></a>
                                                </td>";
                                                echo "</tr>";
                                            }
                                        } else {
                                            echo "<tr>";
                                            echo "<td colspan='12' style='text-align: center;'>Tidak ada data ditemukan</td>";
                                            echo "</tr>";
                                        }
                                        ?>
                                    </tbody>
