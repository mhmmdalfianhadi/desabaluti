...
if (isset($_GET['dari_tanggal']) && isset($_GET['sampai_tanggal']) && !empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])) {
    ...
    // Query laporan berdasarkan tanggal
    $query = "SELECT * FROM sktm s
            JOIN penduduk p ON s.id_penduduk = p.id_penduduk
            WHERE s.tgl_sktm BETWEEN ? AND ?
            ORDER BY s.tgl_sktm DESC";
...
<body>
    ...
    <h2 style="text-align: center;">Laporan SKTM</h2>
