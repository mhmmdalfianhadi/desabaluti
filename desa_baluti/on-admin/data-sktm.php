...
// Query utama dengan JOIN ke tabel penduduk
$query = "SELECT 
    p.nama,
    p.nik,
    p.ttl,
...
if (!empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])) {
    ...
    $query .= " AND s.tgl_sktm BETWEEN '$dari_tanggal' AND '$sampai_tanggal'";
}

// Urutkan berdasarkan tanggal pengajuan terbaru
$query .= " ORDER BY s.tgl_sktm DESC";
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                            ...
                            <form method="GET" action="">
                                <div class="row align-items-end">
                                    ...
                                    <?php if (!empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])): ?>
                                        <div class="col-md-3">
                                            <label>&nbsp;</label>
                                            <a href="sktm-print.php?dari_tanggal=<?= $_GET['dari_tanggal'] ?>&sampai_tanggal=<?= $_GET['sampai_tanggal'] ?>"
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <button class="btn btn-secondary" onclick="window.location.href='form-sktm.php'"><i class="mdi mdi-pen-plus me-1"></i>Buat SKTM</button>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        if ($result->num_rows > 0) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                echo "<td>" . (!empty($row['tgl_sktm']) ? date("d-m-Y", strtotime($row['tgl_sktm'])) : '') . "</td>";
                                                echo "<td>" . (isset($row['ket']) ? htmlspecialchars($row['ket']) : '-') . "</td>";
                                                echo "<td>
                                                    <a href='edit-sktm.php?id_sktm=" . $row['id_sktm'] . "' class='btn btn-primary mr-1'><i class='fa fa-pencil'></i></a>
                                                    <a href='hapus-sktm.php?id_sktm=" . $row['id_sktm'] . "' class='btn btn-danger mr-1 ' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'><i class='fa fa-trash'></i></a>
                                                    <a href='print-sktm.php?id_sktm=" . $row['id_sktm'] . "' class='btn btn-success mr-1' target='_blank' onclick='openPrint(this.href)'><i class='fa fa-print'></i></a>
                                                </td>";
                                                echo "</tr>";
                                            }
                                        } else {
                                            echo "<tr>";
                                            echo "<td colspan='13' style='text-align: center;'>Tidak ada data ditemukan</td>";
                                            echo "</tr>";
                                        }
                                        ?>
                                    </tbody>
