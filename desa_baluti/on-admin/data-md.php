...
if (!empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])) {
    ...
    $query .= " AND m.tanggal BETWEEN '$dari_tanggal' AND '$sampai_tanggal'";
}
...
<head>
    ...
    <title>Desa Baluti - Meninggal Dunia</title>
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                            ...
                            <form method="GET" action="">
                                <div class="row align-items-end">
                                    ...
                                    <?php if (!empty($_GET['dari_tanggal']) && !empty($_GET['sampai_tanggal'])): ?>
                                        <div class="col-md-3">
                                            <label>&nbsp;</label>
                                            <a href="md-print.php?dari_tanggal=<?= $_GET['dari_tanggal'] ?>&sampai_tanggal=<?= $_GET['sampai_tanggal'] ?>"
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="mb-0">Data Kematian</h5>
                                <button class="btn btn-secondary" onclick="window.location.href='form-md.php'"><i class="mdi mdi-pen-plus me-1"></i>Buat Surat Kematian</button>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        if ($result->num_rows > 0) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                echo "<td>" . htmlspecialchars($row['disebabkan']) . "</td>";
                                                echo "<td>" . (isset($row['no_surat']) ? htmlspecialchars($row['no_surat']) : '-') . "</td>";
                                                echo "<td>" . (isset($row['ket']) ? htmlspecialchars($row['ket']) : '-') . "</td>";
                                                echo "<td>
                                                    <a href='edit-md.php?id_md=" . $row['id_md'] . "' class='btn btn-primary mr-1'><i class='fa fa-pencil'></i></a>
                                                    <a href='hapus-md.php?id_md=" . $row['id_md'] . "' class='btn btn-danger mr-1 ' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'><i class='fa fa-trash'></i></a>
                                                    <a href='print-md.php?id_md=" . $row['id_md'] . "' class='btn btn-success mr-1' target='_blank' onclick='openPrint(this.href)'><i class='fa fa-print'></i></a>
                                                </td>";
                                                echo "</tr>";
                                            }
                                        } else {
