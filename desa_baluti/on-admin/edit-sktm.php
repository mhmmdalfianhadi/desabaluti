...
$id_sktm = isset($_GET['id_sktm']) ? intval($_GET['id_sktm']) : 0;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $tgl_sktm = !empty($_POST['tgl_sktm']) ? date('Y-m-d', strtotime($_POST['tgl_sktm'])) : NULL;
    $ket = $_POST['ket'];

    // Update data ke dalam database berdasarkan id_sktm
    $stmt = $koneksi->prepare("UPDATE sktm SET id_penduduk = ?, keperluan = ?, no_surat = ?, tgl_sktm = ?, ket = ? WHERE id_sktm = ?");
    $stmt->bind_param("issssi", $id_penduduk, $keperluan, $no_surat, $tgl_sktm, $ket, $id_sktm);

    if ($stmt->execute()) {
        echo "<script>
            alert('Data berhasil diperbarui.');
            window.location.href = 'data-sktm.php';
        </script>";
    } else {
        echo "<script>
    ...
    $stmt = $koneksi->prepare("SELECT * FROM sktm WHERE id_sktm = ?");
                                ...
                                <div class="row g-4">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-2">
                                            ...
                                            <input class="form-control" type="text" name="keperluan" value="<?php echo htmlspecialchars($keperluan); ?>" placeholder="Masukkan keperluan" required />
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-2">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-2">
                                            <label for="tgl_sktm" class="form-label">Tanggal SKTM</label>
                                            <input class="form-control" type="date" name="tgl_sktm"
                                                value="<?= htmlspecialchars($tgl_sktm) ?>" required />
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-2">
                                    ...
                                    <div class="pt-4">
                                        ...
                                        <button type="button" class="btn btn-danger me-sm-3 me-1" onclick="window.location.href='data-sktm.php';">Batal</button>
                                    </div>
                                </div>
                            </form>
                        </div>
